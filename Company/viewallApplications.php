...
$cid=$data['id'];
$Datalist=array();
$query=mysqli_query( $con,"SELECT * FROM tbl_jobapply a INNER JOIN tbl_jobseeker s ON a.jobseeker_id=s.jobseeker_id INNER JOIN tbl_prisoner p ON s.prisoner_id=p.prisoner_prisonercode INNER JOIN tbl_jobvaccancy v on a.jobvac_id=v.jobvac_id INNER JOIN tbl_jobdetail d on v.jobdet_id=d.jobdet_id INNER JOIN tbl_company c on v.company_id=c.company_id WHERE a.jobapplier_status=0 AND c.company_id='$cid'");
